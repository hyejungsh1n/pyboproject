...
def delete_question(comment_id):
    ...
    if g.user != comment.user:
        flash('삭제 권한이 없습니다')
        return redirect(url_for('question.detail', question_id=qustion.id))
...
def modify_answer(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            comment.modify_date = datetime.now() #수정일시 저장
            db.session.commit()
            return redirect('{}#comment_{}'.format(url_for('question.detail', question_id=content.answer.question.id), comment.id))
    else:
        form = CommentForm(obj=comment)
    return render_templates('comment/comment_form.html', form=form)
...
def delete_answer(comment_id):
    ...
    if g.user != comment.user:
        flash('삭제 권한이 없습니다')
        return redirect(url_for('qustion.datail', question_id=question_id))
    db.session.delete(comment)
    db.session.commit()
    return redirect(url_for('question.detail', question_id=question_id))